...
//getAll yn Single, un alta de productos y un modificar productos
const {validateCreate, validateModify} = require('../middlewares/products')

const all = (req, res) =>  model.getALL().then((response) => res.json(response)).catch((err) => res.status(500).json(err));


const single = (req, res) => model.single(req.params.id).then((response) => res.json(response)).catch((err) => res.status(500).json(err));
